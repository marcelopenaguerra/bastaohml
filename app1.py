...
with col_disponibilidade:
    ...
    # ========== DEMANDAS PÚBLICAS DISPONÍVEIS ==========
    # Mostrar demandas na tela principal (SEMPRE, não só na view de atividades)
    usuario_logado = st.session_state.usuario_logado
    demandas_ativas = [
        d for d in st.session_state.get('demandas_publicas', []) 
        if d.get('ativa', True) and (
    ...
    if demandas_ativas:
        ...
        .badge-media { background: #f59e0b; }
        .badge-baixa { background: #10b981; }
        .demanda-setor { color: #64748b; font-size: 0.75rem; }
        .demanda-texto { color: #1e293b; font-size: 0.85rem; margin: 0.25rem 0; }
        </style>
        """, unsafe_allow_html=True)
        ...
        for dem in demandas_ativas[:3]:
            ...
            st.markdown(f"""
            <div class="demanda-card demanda-{prioridade_lower}">
                <div style="display: flex; gap: 0.5rem; margin-bottom: 0.25rem;">
                    <span class="demanda-badge badge-{prioridade_lower}">{prioridade.upper()}</span>
                    <span class="demanda-setor">{setor}</span>
                </div>
                <div class="demanda-texto">{texto_limpo[:80]}</div>
            </div>
            """, unsafe_allow_html=True)
            
            if st.button(f"✅ Assumir", key=f"assumir_dem_{dem['id']}", use_container_width=True):
                atividade_desc = f"[{setor}] {texto_limpo[:100]}"
                st.session_state.demanda_start_times[usuario_logado] = now_brasilia()
                status_atual = st.session_state.status_texto.get(usuario_logado, '')
                if status_atual and 'Atividade:' in status_atual:
                    st.session_state.status_texto[usuario_logado] = f"{status_atual} | {atividade_desc}"
                else:
                    st.session_state.status_texto[usuario_logado] = f"Atividade: {atividade_desc}"
                if usuario_logado in st.session_state.bastao_queue:
                    st.session_state.bastao_queue.remove(usuario_logado)
                st.session_state[f'check_{usuario_logado}'] = False
                check_and_assume_baton()
                dem['ativa'] = False
                save_admin_data()
                save_state()
